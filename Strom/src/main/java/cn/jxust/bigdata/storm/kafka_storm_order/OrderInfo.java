...
/**
 * 订单javabean
 * 简单的订单信息，实际业务更复杂，比如父子订单，一个订单多个商品，一个支付订单多个订单等情况
 * 因为生产者要发送订单信息到kafka，经过网络传输要实现序列化接口
 */
public class OrderInfo implements Serializable {
    ...
    public String random() {
    	//实际业务中，会将商品信息进行分类 如1-10w是电器 11-30w是食品等等。。
        this.productId = "12121212";//商品编号
        this.orderId = UUID.randomUUID().toString().replaceAll("-", "");//订单编号
        this.paymentId = UUID.randomUUID().toString().replaceAll("-", "");//支付编号
        this.productPrice = new Random().nextInt(1000);//商品价钱
        this.promotionPrice = new Random().nextInt(500);//优惠价
        this.payPrice = new Random().nextInt(480);//订单支付价格

        String date = "2018-2-11 12:22:12";
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            this.createOrderTime = simpleDateFormat.parse(date);//订单生成时间
        } catch (ParseException e) {
            e.printStackTrace();
        }
        //将javabean对象转为json，生产到kafka
